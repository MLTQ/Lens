...
| `bonsai_lens/attention.py`, `/api/attention`, `/api/attn_cell`, `/api/kv`, `static/kv.js` | Attention and memory, decomposed exactly per (head, source): softmax attention in the 16 full-attention layers and the Gated DeltaNet memory in the other 48 (unrolled into signed effective weights w[t,s] = β_s k_sᵀ(A_{s+1}…A_t) q_t, including decay and delta-rule overwrites). Summed back, contributions reproduce every layer's output (≤0.05% error). Views: arcs into the selected cell in the Grid and J-Volume (per head, or merged), a side-panel breakdown with each source's message decoded by the J-lens, an *attention flow* mode in the J-Volume (each layer's strongest cross-position writes, top messages labelled), and a KV tab: by default every head of the layer as a small hyperbolic disk seen down the query (`/api/kv_heads`; centre = the query's tip, Poincaré radius r = (1−√w)/(1+√w) so each e-fold less attention is an equal step out and the rim is zero, angle = how a key deviates from the query); click a disk for that head's number line (x = match, size = weight actually used) or 3D view. ▶ / the scrubber step the query through the prompt at the selected layer in any of these views (`/api/kv_track` for one head; key directions are fixed per head over the whole sequence so keys glide rather than jump). |
